...
var WorldClient = (function (_super) {
    ...
    WorldClient.prototype.currentlySynced = function (name) {
        return bluebird.resolve(this._datarepo.getData(name));
    };
    ...
    WorldClient.prototype.syncDataDiff = function (name, diff) {
        return diff.length > 0 ? this._promiseEvent(pxtCloudApi.Events.WorldSyncDataDiff, { name: name, diff: diff }) : bluebird.resolve();
    };
    WorldClient.prototype._onConnect = function (socket$$1) {
        _super.prototype._onConnect.call(this, socket$$1);
        this._onNotifyReceivedEvent(pxtCloudApi.Events.WorldSyncData, socket$$1);
        this._onNotifyReceivedEvent(pxtCloudApi.Events.WorldSyncDataDiff, socket$$1);
    };
    WorldClient.prototype._notifyEvent = function (event) {
        ...
        switch (event) {
            case pxtCloudApi.Events.WorldSyncData: {
                var _a = args[0], name_1 = _a.name, data = _a.data;
                if (this._datarepo.isDataSource(name_1)) {
                    this._datarepo.setData(name_1, pxtCloudApi.DataRepo.decode(data));
                }
                break;
            }
            case pxtCloudApi.Events.WorldSyncDataDiff: {
    ...
    WorldClient.prototype._onDisconnect = function (socket$$1) {
        this._offNotifyReceivedEvent(pxtCloudApi.Events.WorldSyncData, socket$$1);
        this._offNotifyReceivedEvent(pxtCloudApi.Events.WorldSyncDataDiff, socket$$1);
    };
    return WorldClient;
}(client_1.Client));
